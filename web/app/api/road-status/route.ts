...
export async function POST(request: Request) {
  ...
  // Close the whole SPAN, not the single edge under the cursor. road_edges holds each
  // carriageway of a divided road separately, so blocking one leaves the twin open and
  // pgr_dijkstra just hops across — the route comes back unchanged and the coordinator's
  // tap silently did nothing. set_span_impassable returns the ids it actually touched.
  const { data: spanIds, error } = await admin.rpc('set_span_impassable', {
    p_edge_id: edgeId,
  ...
  const edgeIds: number[] = Array.isArray(spanIds) ? spanIds.map(Number) : [edgeId];

  // Active dispatch routes are not regenerated by the pipeline (it only touches 'planned').
  // Rebuild them against the new edge costs so a closed road makes them visibly redraw.
  // The edge write above already succeeded, so a reroute failure must not report the block
  // as failed; it is named in `note` and logged instead of being swallowed.
  let reroutedActive = 0;
  let note: string | undefined;
  const { data: n, error: reErr } = await admin.rpc('reroute_active_dispatch_routes');
  if (reErr) {
    console.error('reroute_active_dispatch_routes failed', reErr);
    note = `active dispatch routes not redrawn — ${reErr.message}`;
  } else if (typeof n === 'number') {
    reroutedActive = n;
  }

  return Response.json({ ok: true, edgeId, edgeIds, impassable, reroutedActive, ...(note ? { note } : {}) });
}
