...
export async function GET() {
  ...
  if (cache && now - cache.at < ttlMs) {
    return Response.json({ available: true, conditions: cache.value });
  }

  const base = process.env.OPEN_METEO_BASE_URL ?? 'https://api.open-meteo.com/v1';

  try {
    // Sample the Cebu region in parallel; tolerate individual point failures and take
    // the worst-case wind across whatever points responded.
    const settled = await Promise.allSettled(
      CEBU_SAMPLE_POINTS.map(async (p) => {
    ...
    if (cache) {
      return Response.json({ available: true, conditions: { ...cache.value, stale: true } });
    }
